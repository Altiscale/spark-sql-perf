...
avgpaid as
(select 0.05*avg(netpaid) as av from ssales)
 select c_last_name, c_first_name, s_store_name, sum(netpaid) paid
 from ssales
 left join avgpaid
 where i_color = 'pale'
 group by c_last_name, c_first_name, s_store_name, avgpaid.av
 having sum(netpaid) > 0.05*avgpaid.av
...
--q30.sql--
 ...
 avg_returns as
  (select ctr_state as ar_ctr_state, avg(ctr_total_return)*1.2 av
               from customer_total_return group by ctr_state)
 select c_customer_id,c_salutation,c_first_name,c_last_name,c_preferred_cust_flag
       ,c_birth_day,c_birth_month,c_birth_year,c_birth_country,c_login,c_email_address
       ,c_last_review_date,ctr_total_return
 from customer_total_return ctr1, customer_address, customer, avg_returns
 where ctr1.ctr_total_return > avg_returns.av
       and ctr1.ctr_state = avg_returns.ar_ctr_state
...
--q42.sql--
 ...
 select dt.d_year, item.i_category_id, item.i_category, sum(ss_ext_sales_price) sum_price
 from     date_dim dt, store_sales, item
 where dt.d_date_sk = store_sales.ss_sold_date_sk
     and store_sales.ss_item_sk = item.i_item_sk
...
--q45.sql--
 ...
 from web_sales JOIN customer ON
...
date_bound as 
(select distinct d_month_seq that_month
...
my_revenue as (
 ...
 from   my_customers,
        ...
        store,
        date_dim
    left join date_bound
 where  c_current_addr_sk = ca_address_sk
        and ca_county = s_county
        and ca_state = s_state
        and ss_sold_date_sk = d_date_sk
        and c_customer_sk = ss_customer_sk
        and d_month_seq between date_bound.that_month+1 and date_bound.that_month+2
...
WHERE d.d_date between '1999-02-01' and '1999-04-02' and
               ca.ca_state = 'IL' and
...
select i_item_desc, i_item_id, i_category, i_class, i_current_price
...
--q99.sql--
 ...
 select
    substr(w_warehouse_name,1,20) as warehouse_name, sm_type, cc_name
